...
<body>
<div id="toolbar">
  <h1 id="ttl">Trace — __FILENAME__</h1>
  <button id="bLoad">Load JSON…</button>
  <input type="file" id="fileIn" accept=".json" style="display:none">
  <button id="bFit">Fit (F)</button>
  <button id="bZI">Zoom In (W/+)</button>
  <button id="bZO">Zoom Out (S/−)</button>
  <span id="status"></span>
</div>
<div id="outer">
...
document.addEventListener('keydown', e => {
  /* Ignore shortcuts while typing in any text input */
  if (['INPUT', 'TEXTAREA', 'SELECT'].includes(e.target.tagName)) return;
  const panStep = (cv.width - LW) * 0.15;
  switch (e.key) {
    /* Zoom — W/S like Perfetto, +/- traditional */
    case 'w': case 'W': case '+': case '=': zoomAt(1.4); break;
    case 's': case 'S': case '-': case '_': zoomAt(1 / 1.4); break;
    /* Pan — A/D like Perfetto, arrow keys traditional */
    case 'a': case 'A': case 'ArrowLeft':  vs -= panStep / vsc; render(); break;
    case 'd': case 'D': case 'ArrowRight': vs += panStep / vsc; render(); break;
...
document.getElementById('bLoad').onclick = () => fileIn.click();
fileIn.onchange = e => {
  ...
  reader.onload = ev => {
    try {
      const trace = JSON.parse(ev.target.result);
      D = processTrace(trace);
      const title = 'Trace \u2014 ' + file.name;
