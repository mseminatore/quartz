...
// ---------------------------------------------------------------------------
// Arduino auto-tune: when building with the Arduino IDE or PlatformIO, apply
// sane architecture-specific defaults before the #ifndef guards below so that
// users do not have to configure anything for a basic project.
// Any of these can still be overridden by defining them before including rtos.h.
// ---------------------------------------------------------------------------
#if defined(ARDUINO)
#  if defined(ARDUINO_ARCH_AVR)
     // ATmega328P (Uno/Nano) has 2 KB SRAM — use minimal settings.
#    ifndef RTOS_STACK_BYTES_PER_WORD
#      define RTOS_STACK_BYTES_PER_WORD 1   // byte-granular stacks on AVR
#    endif
#    ifndef RTOS_TICK_TYPE_16BIT
#      define RTOS_TICK_TYPE_16BIT 1        // 16-bit tick type saves SRAM
#    endif
#    ifndef RTOS_IDLE_STACK_WORDS
#      define RTOS_IDLE_STACK_WORDS 16      // 16 bytes is enough for the idle task
#    endif
#    ifndef RTOS_MAX_TASKS
#      define RTOS_MAX_TASKS 8              // keep TCB pool small
#    endif
